...
class InpaintApp(Frame):
    ...
    def start_inpaint(self):
        if self.chosen_img is not None and self.msk is not None:
            ...
            with tf.Session(graph=graph) as sess:
                ...
                inpaint_loss, gradient = inpaint.get_losses(mask_placeholder, images_placeholder, g_in, g_out, g_loss)
                bests = []
    ...
    def setParams(self):
        ...
        print(self.iters.get())
        self.iters.grid(row=0, column=1, padx=5, pady=3)
        Label(tl, text="Weighted Mask: ").grid(row=1, column=0,padx=5, pady=3)
        self.maskedbool = BooleanVar()
        Radiobutton(tl, text="True", variable= self.maskedbool, value=True).grid(row=1, column=1)
        Radiobutton(tl, text="False", variable= self.maskedbool, value=False).grid(row=1, column=2)
