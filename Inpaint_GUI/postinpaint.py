...
def test():
    src = np.asarray(PIL.Image.open('jk.png'))
    src.flags.writeable = True
    tar = np.asarray(PIL.Image.open('ae.png'))
    tar.flags.writeable = True
    m = np.asarray(PIL.Image.open('mask.png'))
    m.flags.writeable = True

    blended = blend(tar, src, m)

    final = toimage(blended)
    final.save('blended.jpg')
